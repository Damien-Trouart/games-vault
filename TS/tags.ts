...
export async function updateTags() {
    ...
    if (response.ok) {
        ...
        const tagsCountEntries: [string, number][] = Object.entries(tagsCount).sort((a, b) => {
            return b[1] - a[1];
        });
        const topTags: [string, number][] = tagsCountEntries.slice(0, 5)
        const topTagsName: string[] = topTags.map(([tag]) => {
            return tag
        })
        ...
        */
        function createTagList(tagsInDisplay: string[]) {
            ...
            const generatedTags: HTMLInputElement[] = Array.from(document.querySelectorAll('.tag'));
            generatedTags.forEach((generatedTag: HTMLInputElement) => {
        ...
        const tagSearchInput: HTMLInputElement | null = document.querySelector('.tagSearch-input');
        const tagSearchBtn: HTMLButtonElement|null = document.querySelector('.tagSearch-btn');
        ...
        function TagSearchResult() {
            const searchCleaned: string = tagSearchInput!.value.trim().toLowerCase();
            if (searchCleaned.length <= 1) {
                createTagList(topTagsName);
            } else {
                const filteredSearch: string[] = Object.keys(tagsCount).filter((tag) => {
